...
def correct_stockouts(daily: pd.DataFrame) -> pd.DataFrame:
    # Ожидаемый спрос в день дефицита — среднее 14 предыдущих надёжных дней (без дефицита и аномалий).
    # Среднее, а не медиана: у редко продаваемого товара медиана равна 0 и упущенный спрос терялся бы.
    result = daily.copy()
    valid = ~(result.stockout_flag | result.is_outlier | result.is_large_client_order)
    reliable = result.quantity_clean.where(valid)
    expected = reliable.groupby(result.sku).transform(lambda s: s.shift(1).rolling(14, min_periods=1).mean())
    fallback = reliable.groupby(result.sku).transform(lambda s: s.shift(1).expanding(min_periods=1).mean())
