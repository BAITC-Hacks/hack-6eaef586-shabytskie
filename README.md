...
В `tests/test_ordering.py` каждая проверка из раздела Must have оформлена отдельным тестом. В `tests/test_pipeline.py` проверяются очистка данных, отсутствие утечки будущего и форматы входных файлов. Всего 38 тестов, включая проверки неизвестного остатка, повторных складских снимков, диапазона планового роста и ручного утверждения.
...
|---|---|---:|---:|
| SKU003 | разовый заказ 600 ед. | 798 | 160 |
| SKU005 | дефицит, флаг в продажах | 199 | 230 (критично) |
| SKU010 | дефицит из отдельной таблицы, 90 ед. в пути | 1001 | 1531 (критично) |

Итого: 10 позиций «критично», 4 «высокая», 6 «не требуется».
