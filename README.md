...
1. Закоммитьте и запушьте проект на GitHub. В репозитории должны быть `app.py`, `requirements.txt`, `src/`, `data/raw/` (демо-данные) и `.streamlit/config.toml`.
...
В `tests/test_ordering.py` каждая проверка из раздела Must have оформлена отдельным тестом. В `tests/test_pipeline.py` проверяются очистка данных, отсутствие утечки будущего и форматы входных файлов. Всего 37 тестов.
