...
from src.ordering import EXPORT_COLUMNS
...
with st.sidebar:
    st.header('Расчёт')
    sales = st.text_input('История продаж', 'data/raw/synthetic_sales.csv')
    suppliers = st.text_input('Справочник поставщиков', 'data/raw/suppliers.csv')
    catalog = st.text_input('Номенклатура', 'data/raw/catalog.csv')
    stock = st.text_input('Текущие остатки (необязательно)', '')
    stockouts = st.text_input('Периоды отсутствия', 'data/raw/stockouts.csv')
    growth = st.text_input('Прогноз прироста', 'data/raw/category_growth.csv')
...
with tab_orders:
    ...
    if st.button(f'Утвердить заказ: {supplier}', disabled=not approver.strip(), key='approve'):
        ...
        stem = APPROVED / f"{''.join(c if c.isalnum() else '_' for c in supplier)}_{datetime.now():%Y%m%d_%H%M}"
